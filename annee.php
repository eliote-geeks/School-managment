...
if(isset($_POST['valid'])){
  if(isset($_POST['annee'],$_POST['cycle']) AND !empty($_POST['annee']) AND !empty($_POST['cycle'])){    
    $annee = htmlspecialchars($_POST['annee']);
    $cycle = htmlspecialchars($_POST['cycle']);
    $req = $bdd->prepare('SELECT * FROM menbre RIGHT JOIN cycle ON cycle.nom = menbre.cycle  WHERE cycle.nom = ? AND menbre.date_enreg LIKE "%'.$annee.'%" ORDER BY menbre.id DESC');    
...
<body>
              ...
              <div class="panel-body">
                <form class="form-inline" role="form" method="post">
                  ...
                  <div class="form-group">
                    ...
                    <input type="annee" class="form-control" name="annee" id="exampleInputEmail2" placeholder="Enter year">
                  </div>
  ...
  <table class="table" >
    <tr><?php if(isset($m)){ ?> <label><?= time()?></label> <?php } ?>
      <th>matricule</th>
      <th>nom</th>
      <th>prenom</th>
